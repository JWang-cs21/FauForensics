...
class ColorJitter(object):
    ...
    def get_params(brightness, contrast, saturation, hue):
        ...
        if brightness is not None:
            ...
            transforms.append(torchvision.transforms.Lambda(lambda img: TTF.adjust_brightness(img, brightness_factor)))

        if contrast is not None:
            contrast_factor = random.uniform(contrast[0], contrast[1])
            transforms.append(torchvision.transforms.Lambda(lambda img: TTF.adjust_contrast(img, contrast_factor)))

        if saturation is not None:
            saturation_factor = random.uniform(saturation[0], saturation[1])
            transforms.append(torchvision.transforms.Lambda(lambda img: TTF.adjust_saturation(img, saturation_factor)))

        if hue is not None:
            hue_factor = random.uniform(hue[0], hue[1])
            transforms.append(torchvision.transforms.Lambda(lambda img: TTF.adjust_hue(img, hue_factor)))
