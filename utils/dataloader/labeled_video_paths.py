...
# /home/ubuntu/anaconda3/envs/va/lib/python3.8/site-packages/pytorchvideo/data/labeled_video_paths.py
        ...
        with g_pathmgr.open(file_path, "r") as f:
            for path_label in f.read().splitlines():
                ...
                if len(line_split) == 1:
                    ...
                    file_path, label, label_v, label_a, start_time = line_split

# ID, label_total, modify_video, modify_audio, start_sec
                video_paths_and_label.append((file_path, int(label), int(label_v), int(label_a), float(start_time)))
        assert (
    ...
    def __getitem__(self, index: int) -> Tuple[str, int]:
        ...
        path, label, label_v, label_a, start_time = self._paths_and_labels[index]

        return (os.path.join(self._path_prefix, path), {"label": label, "label_v": label_v, "label_a": label_a, "start_time": start_time, "path":path})
