...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('##cl, cl_edge: ', cl.size(), cl_edge.size())
        # return cl, cl_edge
        return cl
...
#         # print('##sc:',sc.size())
#         # print('##cl:',cl.size())
#         # print('##mul:',(cl * sc.view(1, n, c)).size())
        
#         cl = (cl * sc.view(1, n, c)).sum(dim=-1)
#         # print('##cl sum:',cl.size())
#         return cl

# num_classes=8, backbone='resnet50', neighbor_num=3, metric="dots"
class MEFARG(nn.Module):
    def __init__(self, num_classes=8, backbone='swin_transformer_base', neighbor_num=4, metric='dots'):
    # def __init__(self, num_classes=8, backbone='resnet50', neighbor_num=3, metric='dots'):
        ...
        # self.head = torch.nn.AdaptiveAvgPool2d 
    ...
    def forward(self, x):
        ...
        # print('##global_linear:',x.size())


        return x.permute(0,2,1)
        # return x.permute(0,2,1) #, self.head(x)
        # return x.permute(0,2,1), self.head(x)
...
if __name__=="__main__":
    ...
    net = MEFARG(num_classes=8, backbone='resnet50', neighbor_num=4, metric="dots")

    state_dict_au = torch.load('/opt/data/private/code/MMDFD/checkpoints/pretrain/au_112.pth')['state_dict']
    net.load_state_dict(state_dict_au, strict=False)
# , map_location='cpu'
    ...
    net.eval().cuda()
    BS=8
    # imgs = torch.rand(BS,3, 224, 224).cpu()
    imgs = torch.rand(BS, 3, 160, 160)
    imgs = torch.rand(BS, 3, 140, 140)
    imgs = torch.rand(BS, 3, 112, 112).cuda()
    out, score = net(imgs)
    print('##out:', out.size())
