...
# https://dl.fbaipublicfiles.com/pytorchvideo/model_zoo/kinetics/CSN\_32x2\_R101.pyth
...
def create_csn(
    *,
    ...
    # Model configs.
    # model_depth: int = 50,
    model_depth: int = 101,         ## changed by wj
    ...
    # Stem configs.
    stem_dim_out: int = 64,     
    # stem_dim_out: int = 24,        ## changed by wj
    ...
    # stage_dim_in_list = [64, 256, 512, 1024]
    # stage_dim_out_list = [256, 512, 1024, 768]
    # Create each stage for CSN.
    for idx in range(len(stage_depths)):

        # stage_dim_in = stage_dim_in_list[idx]       ## changed by wj
        # stage_dim_out = stage_dim_out_list[idx]     ## changed by wj
        # print('##stage_dim_in:',stage_dim_in)
        # print('##stage_dim_out:',stage_dim_out)
    ...
    # blocks.append(LambdaModule(lambda x: x.mean(dim=(4))))   ##changed by wj
...
if __name__ == '__main__':
    x = torch.rand(8,3,25,160,160) 
    print('##input:',x.size())
    # net = create_csn()
    net = csn_temporal_no_head()
    # print(net)
    for key, value in net.state_dict().items():
        print(key)    
    out = net(x)
    print('##out:',out.size())
# cnn_encoder.blocks.4.res_blocks.0.branch1_norm.weight   2048-->768 
# cnn_encoder.blocks.4.res_blocks.0.branch2.norm_c.weight
# cnn_encoder.blocks.4.res_blocks.1.branch2.norm_c.weight


# cnn_encoder.blocks.4.res_blocks.0.branch2.norm_a.weight  512 -->192
# cnn_encoder.blocks.4.res_blocks.0.branch2.norm_b.weight  512 -->192
# cnn_encoder.blocks.4.res_blocks.1.branch2.norm_a.weight
# cnn_encoder.blocks.4.res_blocks.1.branch2.norm_b.weight
# cnn_encoder.blocks.4.res_blocks.2.branch2.norm_a.weight
# cnn_encoder.blocks.4.res_blocks.2.branch2.norm_b.weight